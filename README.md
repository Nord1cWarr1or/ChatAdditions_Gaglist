...
- 10 цветовых тем на выбор (Default, Forest, Sunset, Royal, Rose, Ocean, Crimson, Lavender, Amber, Mint)
- Адаптивный дизайн для мобильных устройств
- CSRF-защита на формах
- Мультиязычная локализация (автоопределение по браузеру, свитчер в шапке)
- Легко добавить новый язык через Pull Request
...
├── lang.php       # Загрузка локализации из JSON, определение языка
├── lang.json      # Переводы (meta + ключи на всех языках)
├── theme.php      # Управление темами (выбор, переключение, загрузка CSS)
├── footer.php     # Общий подвал с переключателем тем и dark/light
├── style.css      # Стили (структура, компоненты)
├── themes/        # Цветовые темы
│   ├── default.css   # Тема по умолчанию (голубой)
│   ├── forest.css    # Зелёная
│   ├── sunset.css    # Янтарная
│   ├── royal.css     # Индиго
│   ├── rose.css      # Розовая
...
Все переводы хранятся в файле `lang.json`. Чтобы добавить новый язык:

1. Откройте `lang.json`
2. Добавьте запись в блок `meta`:
   ```json
   "de": { "name": "Deutsch", "flag": "🇩🇪" }
   ```
3. Добавьте блок переводов с теми же ключами:
   ```json
   "de": {
     "nav_title": "Gag <span>List</span>",
     "nav_add": "Gag hinzufügen",
     ...
   }
   ```
4. Отправьте Pull Request

Если ключ отсутствует в текущем языке, используется русский перевод.

## Темы

Панель поддерживает 10 цветовых тем, каждая с вариациями light/dark. Тема сохраняется в `localStorage`.

### Доступные темы

| Тема | Акцент | Описание |
|------|--------|----------|
| Default | `#0891b2` | Голубой (по умолчанию) |
| Forest | `#15803d` | Зелёный |
| Sunset | `#d97706` | Янтарный |
| Royal | `#6366f1` | Индиго |
| Rose | `#ec4899` | Розовый |
| Ocean | `#2563eb` | Синий |
| Crimson | `#e11d48` | Красный |
| Lavender | `#7c3aed` | Фиолетовый |
| Amber | `#b45309` | Золотой |
| Mint | `#0891b2` | Бирюзовый |

### Добавление новой темы

1. Создайте файл `themes/yourtheme.css`
2. Определите CSS-переменные для `:root` (светлая) и `body.dark` (тёмная):
   ```css
   :root {
       --accent: #your-accent;
       --bg: #your-bg;
   ...
   ```
3. Добавьте имя темы в массив `$names` в функции `theme_name()` в `theme.php`
4. Отправьте Pull Request

Для создания темы используйте существующие файлы в `themes/` как пример.
